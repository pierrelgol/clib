...
void			*gpa_resize(void *const handle, void *const ptr,
					const uint64_t nsize);
void			*gpa_dup(void *const handle, void *const ptr,
					const uint64_t size);
...
void			*arena_resize(void *const handle, void *const ptr,
					const uint64_t nsize);
void			*arena_dup(void *const handle, void *const ptr,
					const uint64_t size);
...
void			*memresize(t_allocator *const allocator, void *const ptr,
					const uint64_t osize, const uint64_t nsize);
void			*memdup(t_allocator *const allocator, void *const ptr,
					const int64_t size);
...
void			bitset_set_bit(t_bitset *bitset, const uint64_t index,
					const bool value);
...
t_bitset		bitset_difference(const t_bitset *bitset1,
					const t_bitset *bitset2);
...
char			*string_search_scalar(const char *source, const int32_t scalar,
					const uint64_t n);
char			*string_search_any(const char *source,
					t_bitset const *delimiters, const uint64_t n);
char			*string_search_none(const char *source,
					t_bitset const *delimiters, const uint64_t n);
char			*string_search_predicate(const char *source,
					bool(predicate)(int32_t ch), const uint64_t n);
char			*string_search_sequence(const char *haystack,
					const char *needle, const uint64_t n);

bool			string_contains_scalar(const char *source, const int32_t scalar,
					const uint64_t n);
bool			string_contains_any(const char *source,
					t_bitset const *delimiters, const uint64_t n);
bool			string_contains_none(const char *source,
					t_bitset const *delimiters, const uint64_t n);
bool			string_contains_predicate(const char *source,
					bool(predicate)(int32_t ch), const uint64_t n);
bool			string_contains_sequence(const char *needle,
					const char *haystack, const uint64_t n);

bool			string_starts_with_scalar(const char *source,
					const int32_t scalar);
bool			string_starts_with_any(const char *source,
...
bool			string_ends_with_scalar(const char *source,
					const int32_t scalar);
bool			string_ends_with_any(const char *source,
					t_bitset const *delimiters);
bool			string_ends_with_none(const char *source,
					t_bitset const *delimiters);
bool			string_ends_with_predicate(const char *source,
					bool(predicate)(int32_t ch));
bool			string_ends_with_sequence(const char *haystack,
					const char *needle);
...
uint64_t		string_count_any(const char *source, t_bitset const *delimiters,
					const uint64_t n);
uint64_t		string_count_none(const char *source,
					t_bitset const *delimiters, const uint64_t n);
...
uint64_t		string_count_until_scalar(const char *source,
					const int32_t scalar);
...
uint64_t		string_wcount_any(const char *source,
					const t_bitset *delimiters);
uint64_t		string_wcount_none(const char *source,
					const t_bitset *delimiters);
uint64_t		string_wcount_predicate(const char *source,
					bool(predicate)(int32_t ch));
uint64_t		string_wcount_sequence(const char *source,
					const char *sequence);

uint64_t		string_wlength_scalar(const char *source, const int32_t scalar);
uint64_t		string_wlength_any(const char *source,
					const t_bitset *delimiters);
uint64_t		string_wlength_none(const char *source,
					const t_bitset *delimiters);
uint64_t		string_wlength_predicate(const char *source,
					bool(predicate)(int32_t ch));
uint64_t		string_wlength_sequence(const char *source,
					const char *sequence);
...
uint64_t		string_wfind_any(const char *source, const t_bitset *delimiters,
					uint64_t *out_start, uint64_t *out_end);
uint64_t		string_wfind_none(const char *source,
					const t_bitset *delimiters, uint64_t *out_start,
					uint64_t *out_end);
...
char			*string_clone_scalar(t_allocator *const allocator,
					const int32_t ch);
...
char			*string_clone_until_any(t_allocator *const allocator,
					const char *src, const t_bitset *delimiters);
char			*string_clone_until_none(t_allocator *const allocator,
					const char *src, const t_bitset *delimiters);
...
char			*string_substring_scalar(t_allocator *const allocator,
					const char *src, const int32_t scalar);
char			*string_substring_any(t_allocator *const allocator,
					const char *src, const t_bitset *delimiters);
char			*string_substring_none(t_allocator *const allocator,
					const char *src, const t_bitset *delimiters);
...
char			*string_join_scalar(t_allocator *const allocator,
					const char *string, const int32_t scalar);
char			*string_join_sequence(t_allocator *const allocator,
					const char *string, const char *sequence);
...
char			*string_append_many_scalar_sequence(char *dest,
					const int32_t scalar, const char **many,
					const uint64_t destsize);

char			*string_trim_left(t_allocator *const allocator,
					const char *source, const uint64_t amount);
char			*string_trim_right(t_allocator *const allocator,
					const char *source, const uint64_t amount);
char			*string_trim_both(t_allocator *const allocator,
					const char *source, const uint64_t amount);
...
char			*string_filter_scalar(t_allocator *const allocator,
					const char *source, const int32_t scalar);
char			*string_filter_any(t_allocator *const allocator,
					const char *source, t_bitset const *delimiters);
...
char			*string_replace_scalar(t_allocator *const allocator,
					const char *source, const int32_t scalar,
					const int32_t with);
char			*string_replace_any(t_allocator *const allocator,
					const char *source, t_bitset const *delimiters,
					const int32_t with);
char			*string_replace_none(t_allocator *const allocator,
					const char *source, t_bitset const *delimiters,
					const int32_t with);
char			*string_replace_predicate(t_allocator *const allocator,
					const char *source, bool(predicate)(int32_t ch),
					const int32_t with);
char			*string_replace_sequence(const char *haystack,
					const char *needle, const char *with, char *out_buffer);

char			**string_split_scalar(t_allocator *const allocator,
					const char *source, const int32_t scalar);
char			**string_split_any(t_allocator *const allocator,
					const char *source, t_bitset const *delimiters);
char			**string_split_none(t_allocator *const allocator,
					const char *source, t_bitset const *delimiters);
